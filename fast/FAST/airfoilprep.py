...
class Polar(object):
    ...
    def unsteadyparam(self, alpha_linear_min=-5, alpha_linear_max=5):
        ...
        return (0.0, degrees(alphaU), degrees(alpha0), m,
                cnStallUpper, cnStallLower, degrees(alpha[minIdx]), cd[minIdx])
    ...
    def plot(self, fig = None):
        ...
        self.plotax(ax = ax)    
    def plotax(self, ax = None):
        if ax == None:
            fig = plt.figure(facecolor='white')
            ax = fig.add_subplot(111)
        ax.grid(True)
        ax.plot(self.alpha, self.cl, marker='+')
        ax.plot(self.alpha, self.cd, marker='*')
        ax.plot(self.alpha, self.cm, marker='o')
...
#            print >> f, '{0:<10f}\t{1:40}'.format(param[0], 'Control setting')
            print >> f, '{0:<10f}\t{1:40}'.format(0.0, 'Table ID parameter')
